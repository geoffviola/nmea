...
public:
  ...
  inline void VtgCallback(raw_nmea::vtg const &message)
  {
    this->vtgTrueTrackMadeGood = message.true_track_made_good;
  }

  inline LocalPose2_5D GetState()
  {
    std::lock_guard<mutex> lock(this->stateMx);
    this->state.yaw = yaw_ned_to_enu(this->vtgTrueTrackMadeGood);
    return this->state;
  }
...
private:
  ...
  atomic<double> vtgTrueTrackMadeGood;
  LocalPose2_5D state;
  mutex stateMx;
...
int main(int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "gga_and_vtg_to_tf");
  ...
  else
  {
    ...
    if (ros::ok())
    {
      ...
      ros::Subscriber vtg_sub =
          n.subscribe("vtg", 1, &GlobalToLocalSynchronizer::VtgCallback,
